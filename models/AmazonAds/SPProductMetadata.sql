...
{% set result =set_table_name("sp_productmetadata_tbl_ptrn","sp_productmetadata_tbl_exclude_ptrn") %}
...
{% for i in result %}
    ...
    {{unnesting("priceToPay")}}
        {% if is_incremental() %}
            ...
            WHERE a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sp_productmetadata_lookback') }},0) from {{ this }})
        {% endif %}    
