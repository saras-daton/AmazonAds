...
{% set result =set_table_name("sp_portfolio_tbl_ptrn","sp_portfolio_tbl_exclude_ptrn") %}
...
{% for i in result %}
    ...
    {% if is_incremental() %}
        ...
        where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sp_portfolio_lookback') }},0) from {{ this }})
    {% endif %}
