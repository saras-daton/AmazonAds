{% if var('SPPortfolio') %}
    {{ config( enabled = True,
    post_hook = "drop table {{this|replace('SPPortfolio', 'SPPortfolio_temp')}}"
    ) }}
{% else %}
...
{% endif %}
    ...
    {% for i in results_list %}
        ...
        {%- set query -%}
        {{action1}}
        {{tbl|replace('SPPortfolio', 'SPPortfolio_temp')}}
    ...
    {% endfor %}
    select * from {{this|replace('SPPortfolio', 'SPPortfolio_temp')}}    
