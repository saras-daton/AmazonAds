{% if var('SPProductAdsReport') %}
    {{ config( enabled = True,
    post_hook = "drop table {{this|replace('SPProductAdsReport', 'SPProductAdsReport_temp')}}"
    ) }}
{% else %}
...
{% endif %}
    ...
    {% for i in results_list %}
        ...
        {%- set query -%}
        {{action1}}
        {{tbl|replace('SPProductAdsReport', 'SPProductAdsReport_temp')}}
    ...
    {% endfor %}
    select * from {{this|replace('SPProductAdsReport', 'SPProductAdsReport_temp')}}    
