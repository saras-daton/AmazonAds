...
{% set result =set_table_name("sp_searchtermkeyword_tbl_ptrn","sp_searchtermkeyword_tbl_exclude_ptrn") %}
...
{% for i in result %}
    ...
    from {{i}} a
        ...
        where a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('sp_searchtermkeyword_lookback') }},0) from {{ this }})
        {% endif %}            
