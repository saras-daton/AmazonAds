{% if var('SDProductAdsReport') %}
    {{ config( enabled = True,
    post_hook = "drop table {{this|replace('SDProductAdsReport', 'SDProductAdsReport_temp')}}"
    ) }}
{% else %}
...
{% endif %}
    ...
    {% for i in results_list %}
        ...
        {%- set query -%}
        {{action1}}
        {{tbl|replace('SDProductAdsReport', 'SDProductAdsReport_temp')}}
    ...
    {% endfor %}
    select * from {{this|replace('SDProductAdsReport', 'SDProductAdsReport_temp')}}    
