...
{% if var('SBUnifiedAdGroupsReport') %}
    {{ config( enabled = True,
    post_hook = "drop table {{this|replace('SBUnifiedAdGroupsReport', 'SBUnifiedAdGroupsReport_temp')}}"
    ) }}
{% else %}
...
{% endif %}
    ...
    {% for i in results_list %}
        ...
        {%- set query -%}
        {{action1}}
        {{tbl|replace('SBUnifiedAdGroupsReport', 'SBUnifiedAdGroupsReport_temp')}}
    ...
    {% endfor %}
    select * from {{this|replace('SBUnifiedAdGroupsReport', 'SBUnifiedAdGroupsReport_temp')}}    
