{% if var('SPPlacementCampaignsReport') %}
    {{ config( enabled = True,
    post_hook = "drop table {{this|replace('SPPlacementCampaignsReport', 'SPPlacementCampaignsReport_temp')}}"
    ) }}
{% else %}
...
{% endif %}
    ...
    {% for i in results_list %}
        ...
        {%- set query -%}
        {{action1}}
        {{tbl|replace('SPPlacementCampaignsReport', 'SPPlacementCampaignsReport_temp')}}
    ...
    {% endfor %}
    select * from {{this|replace('SPPlacementCampaignsReport', 'SPPlacementCampaignsReport_temp')}}    
