...
{{set_table_name('%sponsoredbrands_placementcampaignsreport')}}    
{% endset %}  
...
{% for i in results_list %}
    ...
    From (
        ...
        ROW_NUMBER() OVER (PARTITION BY reportdate,campaignId,coalesce(placement,'') order by {{daton_batch_runtime()}} desc) row_num
        from {{i}}
            {% if is_incremental() %}
