{% if var('SBPortfolio') %}
    {{ config( enabled = True,
    post_hook = "drop table {{this|replace('SBPortfolio', 'SBPortfolio_temp')}}"
    ) }}
{% else %}
...
{% endif %}
    ...
    {% for i in results_list %}
        ...
        {%- set query -%}
        {{action1}}
        {{tbl|replace('SBPortfolio', 'SBPortfolio_temp')}}
    ...
    {% endfor %}
    select * from {{this|replace('SBPortfolio', 'SBPortfolio_temp')}}    
