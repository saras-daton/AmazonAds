...
{{ config(enabled=var('SponsoredBrands_Campaign', True)) }}
...
where lower(table_name) like '%sponsoredbrands_campaign' 
{% endset %}  
...
{% for i in results_list %}
    ...
    FROM (
        ...
        FROM  {{i}} 
                cross join unnest(creative) creative
                cross join unnest(landingPage) landingPage
                left join unnest(bidAdjustments) bidAdjustments
